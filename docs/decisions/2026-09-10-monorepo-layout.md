...
2. **Frontend-only repository extending the Papership reference in place (Vite web app first, Tauri later).** Rejected: the intake requires Tauri 2 from phase 07 with signed releases and keychain storage (PRD-D.13, NFR-7); the reference's dependency tree (24 audit advisories, heavy unused libraries) would be inherited rather than pruned; `vercel.json` web deployment is not the target.
...
- Positive: one change set can span contracts, API, worker, desktop and infra with a single PR, checks, review evidence and release proposal — exactly the loop release 1 must demonstrate (R1-ACC-6). Path-scoped ownership and CI filters keep the blast radius of agent changes bounded. `packages/ui` isolates the Papership port and its accessibility fixes from view logic, easing NFR-3 fidelity comparison.
