...
| V16-7 | Mobile/PWA clients use the same API and UI contracts; explicit offline; iOS/Android shells wrap `apps/web` | static scan `start_url`; `test_phase7.py`; `apps/mobile/toolchain-report.json` | scan + pytest + toolchain report | R4 / Phase 7 | PARTIAL — PWA + wrap config evidenced; store binaries and simulator screenshots not claimed |
