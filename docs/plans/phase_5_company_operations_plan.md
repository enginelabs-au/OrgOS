...
intake: docs/blueprints/company_agent_system_blueprint.md (Phase 10)
...
- Memory manager UI on blueprint-2 matching `docs/blueprints/ui-blueprint.md` §B.6 (kinds + class filter, list, provenance detail, honest empty). Seat visibility: Founder full; Project Lead domain/project correct/restrict; Operator own sessions/preferences inspect-only.
...
| Adaptive views | Specified in `docs/blueprints/ui-blueprint.md` §D; no schema, store, or renderer |
...
- `docs/blueprints/ui-blueprint.md` §B.6 / §D if implementation reveals a documented extension
