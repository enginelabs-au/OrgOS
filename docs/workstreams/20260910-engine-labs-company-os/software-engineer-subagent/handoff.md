...
| F-S3 | Reference `npm audit`: 24 advisories (1 critical, 11 high) in the Papership dependency tree | Low for phase 0 (reference not shipped); informs D-01 alternative 2 | `security-engineer-subagent` | Do not inherit the reference lockfile; port only `react`/`react-dom`-dependent code | `npm audit` on `apps/desktop` in phase 1 |
