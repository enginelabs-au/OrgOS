...
## Installation of protected root files

Agents cannot create or modify `AGENTS.md`, `.cursorignore`, or `.github/workflows/agent-governance.yml`. When a repository receives the `.cursor/` control plane without them, an accountable human installs them from the canonical source outside the agent tool path, then reruns `node .cursor/skills/launch-pipeline/scripts/preflight.mjs` and `node .cursor/scripts/validate-agent-config.mjs`.
