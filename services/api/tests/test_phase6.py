...
def test_rate_card_is_trial_published(client: TestClient, founder_headers: dict[str, str]) -> None:
    ...
    labels = [row["id"] for row in body["plans"]]
    assert labels == ["free", "pro", "max", "enterprise"]
    tokens = [row["tokens_month"] for row in body["plans"]]
    assert tokens == [50_000, 200_000, 800_000, 400_000]
    assert body["plans"][1]["usd_month"] == 24
    assert body["plans"][2]["usd_month"] == 120
    assert body["plans"][3]["usd_month"] == 32
    assert len(body["usage_tiers"]) == 5
