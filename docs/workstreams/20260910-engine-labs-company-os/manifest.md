...
- UI reference: [enginelabs-au/OrgOS](https://github.com/enginelabs-au/OrgOS) at commit `8a843bd6429faf1ace5a9eb6dcfb7440703d34c4`, active route `/cc-org-dash`. Inspected read-only on 2026-09-10 (see `.cursor/STATE.md` Active Items).
...
| `ui-ux-developer-subagent` | required | Desktop shell derived from OrgOS `/cc-org-dash`; design specification for home, work item, assistant, agent run, connection setup, memory manager, permissions editor; accessibility and responsive states (trigger: user-facing layout/interaction) | `product-manager-subagent` | read-only; lead materializes under `.../ui-ux-developer-subagent/` | CONDITIONAL (2026-09-10T16:31Z) | `ui-ux-developer-subagent/handoff.md` → `docs/ui-blueprint.md` |
...
| GitHub `enginelabs-au/OrgOS` | none (read-only via GitHub MCP / clone) | all | read | always |
...
- No Figma file exists for this product; UI/UX evidence comes from the OrgOS source and captured screenshots. Figma MCP not used unless the owner supplies a file.
